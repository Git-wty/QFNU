...
<%@page import="com.impl.ClassImpl"%>
...
<%
	...
	String Name = hs1.getAttribute("name").toString();
%>
<body>
	<!--顶部导航栏-->
	...
	<section class="class-main">
		...
		<div class="class-content">
			...
			select * from student_class where class_id='<%= classId%>';
			</sql:query>
						<c:forEach var="row" items="${result.rows}">
							<sql:query dataSource="${question}" var="result1">
			select * from user where username='<c:out value="${row.student_id}" />';
		</sql:query>
							<c:forEach var="row1" items="${result1.rows}">
								<tr>
									<th><div class="table-title">
											<c:out value="${row1.name}"></c:out>
										</div></th>
									<c:set var="name" value="${row1.name}"></c:set>
									<%
...
hs1.setAttribute("stuNames", stuNames);

%>
			<div
				style="text-align: center; font-size: 22px; margin-bottom: 20px;">
				班级总人数：<%=stuNames.size()%></div>

			<div>
				<form action="teacherRollCallShowMember.jsp" id="rollcallform">
					<div style="width: 100%; text-align: center;">
						<span style="font-size: 20px;">请输入您要点名的学生数量：</span> <input
							type='text' name='num' id="num" class='layui-input'
							style="display: inline; width: 200px; margin-top: 20px;" /> <br />
						<button id="roll_call" class="layui-btn layui-btn-lg"
							style="display: block; margin: 20px auto 0px auto;">智能随机点名</button>
					</div>
				</form>
...
function back(){
	...
	<script>
			...
			layui.use('element', function() {
			  ...
			  $('#roll_call').on().on('click', function(){
				  ...
				  else if(num > <%=stuNames.size()%> || num < 1){
					  alert("点名人数超出班级范围！");
					return false;
				  }
				  else {
	...
	<script type="text/javascript">
			function rollcall(){
			    ...
			    var msg = new SpeechSynthesisUtterance("<%=stuNames%>");
			    console.log(msg);
			    window.speechSynthesis.speak(msg);
			}
			
			$("#random").click(function (){
				var tr_name = $(".tr_name").text();
				console.log(tr_name);
