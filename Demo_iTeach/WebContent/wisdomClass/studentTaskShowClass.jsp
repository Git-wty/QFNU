				...
				<table class="layui-table">
					...
					<tbody>
						...
						<sql:query dataSource="${teacher_class}" var="result">
	 					SELECT * from student_class WHERE student_id='<%=username%>';
			 	</sql:query>
						<c:forEach var="row" items="${result.rows}">
							<sql:query dataSource="${teacher_class}" var="className">
	 						SELECT * from class WHERE class_id='<c:out
									value="${row.class_id}" />';
			 			</sql:query>
							<c:forEach var="row1" items="${className.rows}">
								<tr>
									<td><a
										href="studentShowTask.jsp?class_id=<c:out value="${row1.class_id}"/>&class_name=<c:out value="${row1.class_name}" />">
											<c:out value="${row1.class_name}" />
									</a></td>
									<td><a
										href="studentShowTask.jsp?class_id=<c:out value="${row1.class_id}"/>&class_name=<c:out value="${row1.class_name}" />">
											<c:out value="${row1.teacher_name}" />
									</a></td>
