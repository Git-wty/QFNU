...
<body>
	...
	String studentId = hs.getAttribute("username").toString();
	String studentName = hs.getAttribute("name").toString();
	...
	<section class="class-main">
		...
		<div class="class-content">
				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse" id="example-navbar-collapse">
						<ul class="nav navbar-nav">
							<li><a href="studentShowClass.jsp">课堂管理</a></li>
				...
				<table class="layui-table">
					...
					<thead>
						<tr>
							...
							<th><div class="table-title">授课老师</div></th>
						</tr>
					</thead>
					<tbody>
					...
					SELECT * From class WHERE class_id in (SELECT class_id from student_class WHERE student_id='<%=studentId%>');
			</sql:query>
						...
						<c:forEach var="row" items="${result.rows}">
							<tr>
								...
								<td><a
									href="studentShowMember.jsp?class_id=<c:out value="${row.class_id}"/>"><c:out
											value="${row.class_name}"></c:out></a></td>
								<td><a
									href="studentShowMember.jsp?class_id=<c:out value="${row.class_id}"/>"><c:out
											value="${row.teacher_name}"></c:out></a></td>
							</tr>
			...
			<form action="../JoinClassServlet" id="joinclassform"></form>
			<button id="join_class" class="layui-btn" style="margin-left: 45%">加入新的课堂</button>
	...
	<script type="text/javascript">
		  ...
		  layer.ready(function() {
			  $('#join_class').on('click', function(){
				  layer.prompt({title: '请输入老师提供的四位课堂号'},function(val, index){
					  layer.msg('加入成功');
					  var $class_id = $("<input type='hidden' name='class_id' value='"+val+"'>");
					  
						 $('#joinclassform').append($class_id);
