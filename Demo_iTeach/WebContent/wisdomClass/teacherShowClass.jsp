...
<body>
	...
	<%
		HttpSession teahs = request.getSession();
		String teacherName = teahs.getAttribute("name").toString();
		String status = teahs.getAttribute("status").toString();
		String studentId = teahs.getAttribute("username").toString();
	%>
	<!--顶部导航栏-->
	<header class="container" style="width: 100%;">
		...
		<div class="header-line1">
			...
			<div class="header-user">
				<a href="../iTeach-myself.jsp"><%=teacherName%></a>
			</div>
	...
	<section class="class-main">
			...
			<ul class="layui-nav layui-nav-tree" lay-filter="test"
				...
				<li class="layui-nav-item layui-nav-itemed"><a
					...
					<dl class="layui-nav-child">
						...
						<dd>
							<a href="javaScript:;" id="courseware_show_class">课件管理</a>
						</dd>
		...
		<div class="class-content">
			...
			<fieldset class="layui-elem-field layui-field-title"
				...
				<legend> 点击您已创建的课堂，查看点名册，进行智能随机点名</legend>
			</fieldset>

			<div style="text-align: center; margin-top: 20px;">
				<h2>创建课堂</h2>
				<form action="../CreateClassServlet" id="createclassform">
					<input type="hidden" name="teacher_name" value='<%=teacherName%>' />
					<div style="width: 100%; text-align: center;">
						<span style="font-size: 20px;">请输入你想要创建的课堂名称：</span> <input
							type='text' name='class_name' id="class_name" class='layui-input'
							style="display: inline; width: 200px; margin-top: 20px;" />
						<button id="create_class" class="layui-btn layui-btn-lg"
							style="margin-left: 20px;">创建课堂</button>
					</div>
				</form>
			...
			<div class="layui-form">
				<table class="layui-table">
					...
					select * from class where teacher_name='<%=teacherName%>';
				</sql:query>

						<c:forEach var="row" items="${result.rows}">
							<tr>
								<td><a
									href="teacherShowMember.jsp?class_id=<c:out value="${row.class_id}"/>"><c:out
											value="${row.class_name}"></c:out></a></td>

								<td><a
									href="teacherShowMember.jsp?class_id=<c:out value="${row.class_id}"/>"><c:out
											value="${row.class_id}"></c:out></a></td>
							</tr>
						</c:forEach>
	...
	<%String rootPath = request.getContextPath();%>
		$('#show_class').on(
				'click',
				function() {
					window.location.replace("<%=rootPath%>"+ "/wisdomClass/teacherShowClass.jsp");
				});

		$('#task_show_class').on(
				'click',
				function() {
					window.location.replace("<%=rootPath%>"+ "/wisdomClass/teacherTaskShowClass.jsp");
				});

		$('#comment_show_class').on(
				'click',
				function() {
					window.location.replace("<%=rootPath%>"+ "/wisdomClass/teacherCommenntShowClass.jsp");
				});
	</script>
	...
	<script type="text/javascript">
		...
		$('#courseware_show_class').on('click', function() {
			window.location.replace("teacherCoursewareShowClass.jsp");
		});
