...
<body>
	...
	<section class="class-main">
		...
		<div class="community-question">
			...
			<div
				style="font-size: 17px; padding: 20px; border: 2px solid deepskyblue">
				...
				<sql:query dataSource="${question}" var="result1">
	 				SELECT * from question WHERE question_id='<%=question_id%>';
				</sql:query>
				<c:forEach var="row" items="${result1.rows}">
					<h2>
						<c:out value="${row.question_name}" />
					</h2>
					<div style="margin-top: 10px;">
						提问人：
						<c:out value="${row.question_user}" />
					</div>
					<div class="question_int">
						问题详情：
						<c:out value="${row.question_introduce}" />
					</div>
				</c:forEach>
				<form action="Insert_question_comment" method="get">
					<textarea class="my_anwser" placeholder="说出你的想法！"
						name="question_comment" id="my_anwser"></textarea>
					<input type="submit"
						style="display: block; width: 160px; text-align: center; margin: 0 auto;"
						class="btn btn-default" value="我来帮他解答"
						onclick="return notEmpty();" /> <input type="text"
						value="<%=question_id%>" style="display: none;" name="question_id" />
				</form>
			</div>
			<sql:query dataSource="${question}" var="result2">
			...
			<c:forEach var="row" items="${result2.rows}">
				<div class="question_comment">
					<div>
						用户：
						<c:out value="${row.comment_user}" />
					</div>
					<div style="margin-top: 8px;">
						解答：
						<c:out value="${row.question_comment}" />
					</div>
				</div>
			</c:forEach>
		</div>
		<div class="community-control">
			...
			<a href="Insert_question.jsp">
				<div class="community-control-button">我要提问</div>
			</a> <a href="iTeach-community-my.jsp">
				<div class="community-control-button">我的问题</div>
			</a>
		</div>
	</section>
				...
				<ul class="nav nav-tabs nav-tabs-justified ">
					<div class="row" align="center">
						<div class="hidden-lg hidden-md hidden-sm col-xs-3" align="center">
							<li><a href="index.html"><img src="img/首页底部.png"
	...
	<script type="text/javascript">
		...
		function notEmpty() {
			var my_anwser=document.getElementById("my_anwser").value;
			if(my_anwser== "") {
				alert("请提出有用的回答呦！");
				return false;
			} else {
