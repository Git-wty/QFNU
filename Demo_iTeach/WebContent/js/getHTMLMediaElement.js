...
function getHTMLMediaElement(mediaElement, config) {
    ...
    if (buttons.has('full-screen')) {
        ...
        function screenStateChange(e) {
			...
			console.log(mediaElementContainer.style.width);
            if (config.height) {
    ...
    if (config.title) {
        ...
        //修改：此处用来为左上角课堂序号进行样式设计
        h2.setAttribute('style', 'position: absolute;color:white;font-size:17px;text-shadow: 1px 1px black;padding:0;margin:0;text-align: left; margin-top: 10px; margin-left: 10px; display: none; border: 0;line-height:1.5;z-index:1;');
...
//修改用于直播间的尺寸
//    mediaElementContainer.style.width =800 + 'px';
...
function getAudioElement(mediaElement, config) {
    ...
    mediaElementContainer.style.width = '800px';
