...
<%
	...
	String live_introduce = request.getParameter("live_introduce");
...
<body>
	...
	<section class="make-center">
		<p>注：请务必保证您在开启界面前已经成功连接摄像头、耳机等录播设备，否则可能出现直播开启失败的情况！</p>
		<!--开启直播间-->
		...
		<div style="margin: 0 auto; width: 1100px; height: 600px;">
			...
			<div id="videos-container"
				style="float: left; width: 800px; height: 600px; background-color: black;"></div>
			<!--评论查看区-->
			<div class="comment-div">
				<p style="font-size: 25px;">直播间互动评论区</p>
				<div>
		...
		<!--直播间底部发送评论区-->
		<div style="text-align: left; font-size: 20px;">
			<span>评论发送区：</span> <input type="text" class="form-control"
				id="comment-content" style="display: inline; width: 700px" />
	...
	<script>
		//自动生成直播间唯一房间码，自动点击开启直播间按钮
			...
			xmlhttp.onreadystatechange = function() {
				...
				xmlhttp.open("GET", "Insert_comment?room_id=<%=room_id%>&content="+$("#comment-content").val()+"&name=老师", true);
				xmlhttp.send();
			...
			setInterval(function() {
				...
				xmlhttp.open("GET", "CommentServlet?room_id=<%=room_id%>", true);
				xmlhttp.send();
			}, 2000);
		</script>
</body>
...
<sql:update dataSource="${data}" var="result">
		INSERT INTO live_room (live_name,live_id,live_introduce,live_teacher) VALUES ("<%=live_name%>","<%=room_id%>","<%=live_introduce%>","<%=name%>");
	</sql:update>
